...
@interface INAppStoreWindowDelegateProxy : NSProxy <NSWindowDelegate> {
    id <NSWindowDelegate> secondaryDelegate;
}
...
+ (NSColor *)windowPatternOverlayColor
{
	...
	#endif
		rep = [[[NSBitmapImageRep alloc] initWithData:layerInBase64] autorelease];
        [layerInBase64 autorelease];
	...
	#endif
        rep = [[NSBitmapImageRep alloc] initWithData:[layer2xInBase64 autorelease]];
...
- (void)drawNativeWindowTitleInRect:(NSRect)titleTextRect
{
    ...
    CGRect titleTextCGRect = NSRectToCGRect(titleTextRect);
	HIThemeDrawTextBox((__bridge CFTypeRef)(window.title), &titleTextCGRect, &titleTextInfo, [[NSGraphicsContext currentContext] graphicsPort], self.isFlipped ? kHIThemeOrientationNormal : kHIThemeOrientationInverted);
}
...
- (void)getTitleFrame:(out NSRect *)frame textAttributes:(out NSDictionary **)attributes forWindow:(in INAppStoreWindow *)window
{
	...
	if (isUsingCustomTitleDrawing)
	{
        ...
        [titleParagraphStyle autorelease];
...
@synthesize titleDivider = _titleDivider;
...
@synthesize drawsTitlePatternOverlay = _drawsTitlePatternOverlay;
...
- (void)dealloc
{
    SafeRelease(_titleBarView);
    SafeRelease(_bottomBarView);
    SafeRelease(_closeButton);
    SafeRelease(_minimizeButton);
    SafeRelease(_zoomButton);
    SafeRelease(_fullScreenButton);
    SafeRelease(_titleFont);
    SafeRelease(_titleBarGradient);
    SafeRelease(_bottomBarGradient);
    SafeRelease(_baselineSeparatorColor);
    SafeRelease(_titleTextColor);
    SafeRelease(_titleTextShadow);
    SafeRelease(_inactiveTitleBarGradient);
    SafeRelease(_inactiveBottomBarGradient);
    SafeRelease(_inactiveBaselineSeparatorColor);
    SafeRelease(_inactiveTitleTextColor);
    SafeRelease(_inactiveTitleTextShadow);
    SafeRelease(_titleBarDrawingBlock);
    SafeRelease(_bottomBarDrawingBlock);
...
- (void)setBottomBarView:(NSView *)bottomBarView
{
	if ((_bottomBarView != bottomBarView) && bottomBarView) {
		...
		_bottomBarView = [bottomBarView retain];
...
- (void)setMinimizeButton:(INWindowButton *)minimizeButton
{
	if (_minimizeButton != minimizeButton) {
		...
		_minimizeButton = [minimizeButton retain];
		if (_minimizeButton) {
...
- (void)setFullScreenButton:(INWindowButton *)fullScreenButton
{
	if (_fullScreenButton != fullScreenButton) {
		...
		_fullScreenButton = [fullScreenButton retain];
...
- (void)_createTitlebarView
{
	...
	INMovableByBackgroundContainerView *container = [[[INMovableByBackgroundContainerView alloc] initWithFrame:NSZeroRect] autorelease];
	...
	_titleBarContainer = [container retain];
...
- (void)_createBottomBarView
{
	...
	[self _recalculateFrameForBottomBarContainer];
	[self.themeFrameView addSubview:container positioned:NSWindowBelow relativeTo:firstSubview];
	_bottomBarContainer = [container retain];
	self.bottomBarView = [[[INBottomBarView alloc] initWithFrame:NSZeroRect] autorelease];
}
